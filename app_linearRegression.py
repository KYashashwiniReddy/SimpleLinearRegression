...
#-----------------------------#
# streamlit run app_linearRegression.py in terminal
#--------------------------------#
# load css#
def load_css(file):
    with open(file) as f:
...
fig,ax=plt.subplots() #ax-for axis
x=df[['total_bill']]
...
c4.metric("Adj_R2 ",f"{adj_r2:.3f}")
st.markdown('</div>',unsafe_allow_html=True)
#----------------------------------------#
# coefficient calculation-slope,intercept #
st.markdown(f"""
    <div class="card">
...
# predictions #
st.markdown('<div class="card">',unsafe_allow_html=True)
st.subheader("Predict TipAmount")
